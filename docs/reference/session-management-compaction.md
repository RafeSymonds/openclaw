...
- **Daily reset** (opt-in; configure `session.reset.mode: "daily"`, default boundary 4:00 AM local time on the gateway host) creates a new `sessionId` on the next message after the reset boundary.
